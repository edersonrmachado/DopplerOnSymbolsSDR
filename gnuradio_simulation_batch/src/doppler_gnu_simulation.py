...
class doppler_simulation(gr.top_block):

    def __init__(self, symbol_cfg, pay_len, doppler_shift, doppler_rate, filename):
        gr.top_block.__init__(self, "Doppler Simulation", catch_exceptions=True)

        
        ##################################################
        # Variables
        ##################################################
        self.symbol = symbol= symbol_cfg.s
        self.soft_decoding  = soft_decoding = False
        self.sf = sf = symbol_cfg.sf
        self.samp_rate = samp_rate=int(symbol_cfg.fs*2)
        self.preamb_len =  preamb_len =8
        self.pay_len = pay_len 
        self.ldro = ldro = False
        self.impl_head =  impl_head = True
        self.has_crc =  has_crc = False
        self.file_to_save_samples = file_to_save_samples = filename
        self.doppler_shift = doppler_shift 
        self.doppler_rate = doppler_rate 
        self.cr = cr = 1
        self.clk_offset = clk_offset = 0
        self.center_freq = center_freq=symbol_cfg.f
        self.bw = bw= int(symbol_cfg.bw)
    ...
    def set_symbol(self, symbol):
        self.symbol = symbol
        self.blocks_vector_source_x_0.set_data([self.symbol]*(self.pay_len+1), [])
